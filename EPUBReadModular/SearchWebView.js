...
function MyApp_RemoveAllUnderlinesForElement(element) {
    if (element) {
        if (element.nodeType == 1) {
            if (element.getAttribute("class") == "MyAppUnderline") {
                var text = element.removeChild(element.firstChild);
                element.parentNode.insertBefore(text,element);
                element.parentNode.removeChild(element);
                return true;
                ...
                var normalize = false;
                for (var i=element.childNodes.length-1; i>=0; i--) {
                    if (MyApp_RemoveAllUnderlinesForElement(element.childNodes[i])) {
                        normalize = true;
                    }
                }
                if (normalize) {
...
function MyApp_RemoveAllUnderlines() {
    MyApp_SearchResultCount = 0;
    MyApp_RemoveAllUnderlinesForElement(document.body);
}

function MyApp_UnderlineAllOccurencesOfStringForElement(element,keyword) {
    if (element) {
        if (element.nodeType == 3) {// Text node
            while (true) {
                var value = element.nodeValue;  // Search for keyword in text node
            ...
            if (element.style.display != "none" && element.nodeName.toLowerCase() != 'select') {
                for (var i=element.childNodes.length-1; i>=0; i--) {
                    MyApp_UnderlineAllOccurencesOfStringForElement(element.childNodes[i],keyword);
                }
            }
        }
    }
}
function MyApp_UnderlineAllOccurencesOfString(keyword) {
    MyApp_RemoveAllUnderlines();
    MyApp_UnderlineAllOccurencesOfStringForElement(document.body, keyword.toLowerCase());
}